...
(function ($) {
	...
	function prepare(data){
		...
		str+= "<button class=\'btn-floating btn-small waves-effect pulse waves-light blue \' onclick=\'modify_student(\"";
		str+=data["username"] + "\")\'><i class=\'material-icons\'>edit</i></button></td><td>";
	...
	$(document).ready(function(){
		$("select").change(function(){
			...
			Array.from(nodes).forEach(
					(nd)=>{
							let toMatch = nd.children[2];
							if((stream!=toMatch.textContent) && (stream!="1")){
								nd.style.display="none";
							}
							else{
								nd.style.display="";
	...
	function delete_teacher(x){
		if(confirm("The corresponding teacher will be deleted. Do you want to continue?")==true){
	...
	function modify_teacher(x){
		window.location.href = "coe_teacher_details.php?u="+encodeURIComponent(x);
	}
	...
	<table class="highlight centered highlight" >
		...
		echo $ans->result[$ct-1]->{'address'};
			...
			echo '</td><td><button class="btn-floating btn-small waves-effect pulse waves-light blue" onclick="modify_teacher(\''.$ans->result[$ct-1]->{'username'}.'\')"><i class="material-icons">edit</i></button>';
