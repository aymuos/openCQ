...
$st = checkSet(['key','username','password','usernameStudent','name','department','registration_no','joining_year','email','contact_no'],1);
if($st[0]===0){
    $result['status']="FAIL";
    $result['comment']=$st[1];
...
try{
    ...
    $query = "SELECT id FROM coe WHERE username=? AND password=? AND isActive='1' LIMIT 1";
    ...
    $q = new Query($query,"s");
    $q->execute([$_POST['usernameStudent']]);
    $exist = $q->data();
    if(!$exist){
        $user = $_POST['usernameStudent'];
        $result['status']="FAIL";
        $result['comment']="incorrect student username: $user";
        echo json_encode($result);
        exit();
    }
     ...
     $q->execute([$_POST['name'],$_POST['joining_year'],$bpy,$_POST['email'],$_POST['registration_no'],$_POST['contact_no'],$_POST['department'],$_POST['usernameStudent']]);
     $result['status']="OK";
