<?php
	...
	if(!isset($_POST["key"])){
	...
	else if(!isset($_POST["usernamecoe"])){
		$status = 'FAIL';
		$comment = 'undefined parameter: usernamecoe';
		$arr = array( 	'status' => $status,
						'comment' => $comment);
		echo json_encode($arr);
		exit();
	}
	else if(!isset($_POST["passwordcoe"])){
		$status = 'FAIL';
		$comment = 'undefined parameter: passwordcoe';
		$arr = array( 	'status' => $status,
						'comment' => $comment);
		echo json_encode($arr);
		exit();
		...
		$sender_username_coe = $_POST['usernamecoe'];
		$sender_password_coe = $_POST['passwordcoe'];
		...
		try{
			...
			$q1->execute([$sender_username_coe]);
			$student = $q1->data();
			if(!$student){
				$status = 'FAIL';
				$comment = 'incorrect usernamecoe: ';
				$arr = array( 	'status' => $status,
								'comment' => $comment.$sender_username_coe);
				echo json_encode($arr);
				exit();
			}
			else if($student[0]['password'] != $sender_password_coe){
				$status = 'FAIL';
				$comment = 'incorrect passwordcoe';
				$arr = array( 	'status' => $status,
								'comment' => $comment);
				echo json_encode($arr);
				exit();
