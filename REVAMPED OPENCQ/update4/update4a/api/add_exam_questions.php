...
//	var_dump($combined);
    ...
    $left = array_udiff($combined,$res,function($a,$b){
        if((int)$a[0]===(int)$b['chapterId'] && (int)$a[1]===(int)$b['id']){
//			echo 'sfsfs';
            return 0;
        }
        else if((int)$a[0]>(int)$b['chapterId']){
			//echo $a[0];
//			echo $b[0];
//			echo 'iont2';
            return +1;
        }
        else if((int)$a[0]===(int)$b['chapterId'] && (int)$a[1]>(int)$b['id']){
//          echo 'sfdfdfffgjklkls';
			return +1;
        }
        else if((int)$a[0]<(int)$b['chapterId']){
//            echo 'sfsfs-1';
...
//			echo 'sfsfs-9';
  ...
  //   var_dump($left);
    ...
    if($left){
        ...
        $question = $left[0][1];
        $result['status']='FAIL';
        $result['comment']="invalid pairtt: ($chapter,$question)";
        echo json_encode($result);
        exit();
    }
