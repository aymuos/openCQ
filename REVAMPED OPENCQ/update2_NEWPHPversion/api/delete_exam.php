...
function validateExam($id){
    ...
    exam WHERE id=? AND isTeacherVisible='1' AND isActive IN ('0','4') LIMIT 1";
    $q = new Query($query,"i");
    $q->execute([$id]);
    $exist = $q->data();
    if(!$exist){
...
try{
    ...
    $query = "UPDATE exam SET isTeacherVisible='0' WHERE id=?";
    $q = new Query($query,"i");
    $q->execute([$input->examId]);
    $ressult['status']='OK';
    $ressult['comment']="exam has been deleted";
