<?php
	...
	$valid = checkSet(['key','username','password','sub_code','chapter_id','ques_id','st','ac','wa1','wa2','wa3'],1);
	if($valid[0]==0){
			...
			if(!$result){
				...
				if(!$data){
					...
					if(!$data){
						...
						$q = new Query("SELECT * FROM question WHERE id = ? AND isActive = '1' AND chapterId = ?","ii");
						$q->execute([$sender_quesId,$sender_chapterId]);
						$data = $q->data();
						if(!$data){
							$status = 'FAIL';
							$comment = 'invalid ques_id: ';
							$arr = array( 	'status' => $status,
											'comment' => $comment.$sender_quesId);
							echo json_encode($arr);
						}
						else{
							try{
								...
								$query ="UPDATE question SET isActive = '0' WHERE id = ?";
								$q = new Query($query,'i');
								$q->execute([$sender_quesId]);
								
								Query::tStop();
								$arr = array ( 'status' => 'OK',
												'comment'=> 'question modified successfully',
												'id' => $name
											);
								echo json_encode($arr);
							}
